...
A função deve receber um grafo representado por uma matriz de adjacência e construir uma árvore de caminhos mínimos. 
Caso desejar, neste exercício é permitida a conversão da matriz de adjacência para lista de adjacência. 
Por fim, a árvore de caminhos mínimos deve ser construída a partir do vértice "0".
...
Output Format
Imprimir a árvore de caminhos mínimos. 
Cada linha deve ser impressa no seguinte formato: "v_i: pai[v_i]".
Para o vértice da árvore que não tiver "pai", deve ser impresso o caractere "-" (traço/sinal de menos). C
omo o algoritmo deve ser iniciada a partir do vértice "0", então a primeira linha sempre será "0: -"
*/
#include <stdio.h>
#include <string.h>
#include <math.h>
...
void dijkstra(GrafoMA *G, int s)
{
    if (valida_vertice(G, s))
    {
        ...
        while (!filaE_vazia(f))
        {
            ...
            for (v = 0; v < G->V; v++)
            {
                if ((G->mat[u][v] != 0) && (G->nafila[v]) && (G->key[u] + G->mat[u][v] < G->key[v])) //soma da chave
                {
