...
Para cada uma das "A" próximas linhas devem der lidos três números inteiros. 
...
void alg_prim(GrafoLA *G, int s)
{
    if(valida_vertice(G, s))
    {
        ...
        while(!filaE_vazia(f))
        {
            ...
            {
                if((G->nafila[aux->key]) && (auxP->key < G->key[aux->key]))
                {
                    G->key[aux->key] = auxP->key;
                    G->pai[aux->key] = u;
                }
