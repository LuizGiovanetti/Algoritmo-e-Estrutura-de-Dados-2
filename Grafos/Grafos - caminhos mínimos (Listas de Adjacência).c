...
A segunda linha contém a quantidade de arestas (A). 
Para cada uma das "A" próximas linhas devem der lidos dois números inteiros. 
...
GrafoLA* iniciar_grafoLA(int v){
    ...
    G->nafila = (int*)calloc(v, sizeof(int));
...
void alg_dijkstra(GrafoLA *G, int s)
{
    if(valida_vertice(G, s))
    {
        ...
        while(!filaE_vazia(f))
        {
            ...
            Cell *auxP = G->peso[u]->head;
            Cell *aux = G->adj[u]->head;
            
            while((aux != NULL) && (auxP != NULL))
            {
                if((G->nafila[aux->key]) && (G->key[u] + auxP->key < G->key[aux->key]))
                {
                    G->key[aux->key] = G->key[u] + auxP->key;
                    G->pai[aux->key] = u;
                }
