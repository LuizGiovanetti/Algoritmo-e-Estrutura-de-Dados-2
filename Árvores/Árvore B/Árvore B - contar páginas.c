/*
Implemente uma função que receba uma árvore B de ordem 5. A função deverá retornar a quantidade de páginas contidas na árvore.
...
Output Format
Imprimir a quantidade de páginas.
*/
#include <stdio.h>
#include <string.h>
#include <math.h>
...
void contar_pag(NodeB *tree, int *contador)
{
    if(tree != NULL)
    {
        *contador += 1;
        ...
        if(tree->filhos[0] != NULL)
        {
            if(tree->filhos[0]->eh_no_folha)
                *contador += tree->nro_chaves + 1;
            else
            {
                for(int i = 0; i <= tree->nro_chaves; i++)
                    contar_pag(tree->filhos[i], contador);
            }
        } 
    }
}
