/*
Implemente uma função que receba uma árvore B de ordem 5. 
A função deverá retornar a quantidade de páginas que contém o número mínimo de elementos.
...
int qnt_min(NodeB *tree)
{
    ...
    if(tree->nro_chaves < (N + 1) / 2)
        contador++; 
    
    for(int i = 0; i <= tree->nro_chaves; i++)
        contador += qnt_min(tree->filhos[i]);
