/*
Implemente uma função que receba, pelo menos, uma árvore rubro-negra. 
Essa função deverá retornar um número >= 0 que represente a diferença de somatórios entre nós pretos e vermelhos. 
Por exemplo, primeiramente deve ser somado os valores dos nós de cada tipo. Em seguinda, calcular a diferença entre essas somas.

Input Format
Na primeira linha deve ser lido o número inteiro N, que é referente à quantidade de nós da árvore. 
...
static int soma_B(NodeVP *tree)
{
    if(tree != NULL)
    {
        return (tree->color == 1) * tree->key + soma_B(tree->left) + soma_B(tree->right);  
    }
    else
        return 0;   
}

static int soma_R(NodeVP *tree)
{
    if(tree != NULL)
    {        
        return (tree->color == 0) * tree->key + soma_R(tree->left) + soma_R(tree->right);
    }
    else
        return 0;
}

int soma_diff(NodeVP *tree)
{
    return abs(soma_B(tree) - soma_R(tree));
}
